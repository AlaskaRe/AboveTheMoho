...
Simple structured Delaunay triangulation in 2D with Bowyer-Watson algorithm.
 ...
 The original version Written by Jose M. Espadero ( http://github.com/jmespadero/pyDelaunay2D )
 which based on code from Ayron Catteau. Published at http://github.com/ayron/delaunay .
 To reach the purpose of drawing the 3D model of the stratum,
 the edge attribute is defined, and the exportVoronoiRegions method was disabled.
...
class Delaunay2D:
    ...
    def addPoint(self, p):
        ...
        boundary = []
        # record the point of the hole which is left by the bad triangles
        boundary_point = []
        ...
        while True:
            ...
            if tri_op not in bad_triangles:
                ...
                e0 = T[(edge+1) % 3]
                e1 = T[(edge-1) % 3]
                boundary.append((e0, e1, tri_op))

                # Add the point to the bounday_point list
                if e0 not in boundary_point:
                    boundary_point.append(e0)
                if e1 not in boundary_point:
                    boundary_point.append(e1)
                ...
                # delete the common edge from the edges list
                startpoint = T[(edge+1) % 3]
                endpoint = T[(edge+2) % 3]
                # for ind ,e in self.edges:
                for e in self.edges:
